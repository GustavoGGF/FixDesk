...
((global, factory) => {
	...
	class BaseComponent extends Config {
		...
		static getInstance(element) {
			return Data.get(getElement(element), BaseComponent.DATA_KEY);
		}
		static getOrCreateInstance(element, config = {}) {
			return (
				BaseComponent.getInstance(element) ||
		...
		static get DATA_KEY() {
			return `bs.${BaseComponent.NAME}`;
		}
		static get EVENT_KEY() {
			return `.${BaseComponent.DATA_KEY}`;
		}
		static eventName(name) {
			return `${name}${BaseComponent.EVENT_KEY}`;
		}
	}
		...
		return () => {
			Object.keys(state.elements).forEach((name) => {
				...
				var styleProperties = Object.keys(
					Object.hasOwn(state.styles, name)
						? state.styles[name]
						: initialStyles[name],
				); // Set all values to an empty string to unset them

				var style = styleProperties.reduce((style, property) => {
					style[property] = "";
					return style;
  ...
  */
	...
	function computeAutoPlacement(state, options) {
		...
		var overflows = allowedPlacements.reduce((acc, placement) => {
	...
	function flip(_ref) {
		...
		if (makeFallbackChecks) {
			...
			var _loop = function _loop(_i) {
				var fittingPlacement = placements.find((placement) => {
	...
	function mergeByName(modifiers) {
		var merged = modifiers.reduce((merged, current) => {
	...
	class Dropdown extends BaseComponent {
		...
		static dataApiKeydownHandler(event) {
			...
			// TODO: v6 revert #37011 & change markup https://getbootstrap.com/docs/5.3/forms/input-group/
			const getToggleButton = Dropdown.matches(SELECTOR_DATA_TOGGLE$3)
				? Dropdown
				: SelectorEngine.prev(Dropdown, SELECTOR_DATA_TOGGLE$3)[0] ||
					SelectorEngine.next(Dropdown, SELECTOR_DATA_TOGGLE$3)[0] ||
	...
	class Offcanvas extends BaseComponent {
		...
		// Static
		static jQueryInterface(config) {
			return Offcanvas.each(function () {
	...
	class ScrollSpy extends BaseComponent {
		...
		// Static
		static jQueryInterface(config) {
			return ScrollSpy.each(function () {
